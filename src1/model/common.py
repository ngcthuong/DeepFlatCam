...
def Raw2Bayer(x, crop_size = cSize, is_rotate = True):
    ...
    #y = torchvision.transforms.RandomRotation(y, degrees = (0.05, 0.05))               
...
if __name__ == '__main__':
    # Testing results 
    fc_noise = Image.open('fc001.png')
    fc_noise2 = torch.unsqueeze(torch.unsqueeze(torch.tensor(np.array(fc_noise)), 0), 0).float()/ (2**16)
    
    #tmp = fc_noise2.view(-1)
    #print(min(tmp),max(tmp))

    y_bayer = Raw2Bayer(fc_noise2)
    tmp = y_bayer.clone().detach().view(-1)
    print(min(tmp),max(tmp))

    y = make_separable(y_bayer)
    tmp = torch.tensor(y).view(-1)
    print(min(tmp),max(tmp))
