...
def flatcamRecOrg(x):
    ...
    # Step 1 Perform mean subtraction ---> skip this part
    rowMeans = torch.mean(x, 3)
    colMeans = torch.mean(x, 2)
    allMean = torch.mean(rowMeans, 2)
    rowMeans_ = torch.unsqueeze(rowMeans, -1).expand(x.size())
    colMeans_ = torch.unsqueeze(colMeans, -1).permute(0, 1, 3, 2).expand(x.size())
    allMean_ = torch.unsqueeze(torch.unsqueeze(allMean, -1), -1).expand(x.size())

    img = (x - rowMeans_) - colMeans_ + allMean_

    # Step 2: Simple Inverse 
    UL  = torch.from_numpy(np.squeeze(mat['UL_all'][:,:]).astype(float)).float()
    DL  = torch.from_numpy(np.squeeze(mat['DL_all'][:,:]).astype(float)).float()
    VL  = torch.from_numpy(np.squeeze(mat['VL_all'][:,:]).astype(float)).float()
    UR  = torch.from_numpy(np.squeeze(mat['UR_all'][:,:]).astype(float)).float()
    DR  = torch.from_numpy(np.squeeze(mat['DR_all'][:,:]).astype(float)).float()
    VR  = torch.from_numpy(np.squeeze(mat['VR_all'][:,:]).astype(float)).float()
    singL  = torch.from_numpy(np.squeeze(mat['singL_all'][:,:]).astype(float)).float()
    singR  = torch.from_numpy(np.squeeze(mat['singR_all'][:,:]).astype(float)).float()

    y = torch.zeros((x.size()[0], 4, Plr.size()[2], Qlr.size()[2])).to(torch.device("cuda"))
    lmdb = 0.0002 
    for i in range(0, x.size()[0]):
        for j in range(0, 4):
            lhs = torch.mm( UL[:, :, j].permute(1, 0).cuda(), torch.mm(x[i, j, :, :], UR[:, :, j].cuda()))
            lhs = torch.mm( DL[:, :, j].permute(1, 0).cuda(), torch.mm(lhs, DR[:, :, j].cuda()))
            rhs = torch.mm(torch.unsqueeze(singL[:, j].cuda() ** 2, 1), torch.unsqueeze(singR[:, j], 1).permute(1, 0).cuda() **2).permute(1, 0) + lmdb 

            y[i, j, :, : ] = torch.mm( VL[:, :, j].permute(1, 0).cuda(), torch.mm(lhs/rhs, VR[:, :, j].cuda()))
        

    # Step 3: Convert to bayer pattern 
    y1 = F.relu(y)               # Remove negative value  --> maybe not necessary 
    y2 = Bayer2RGB(y)   

    return x, img, y, y1, y2  

def flatcamRecSimple(x):
    x = Raw2Bayer(x)
                
    # Step 1 Perform mean subtraction ---> skip this part
    # img = (x - torch.mean(x,2)) - torch.mean(x, 3) + torch.mean(torch.mean(x,2), 2)
...
class FlatCamSimInitFix(nn.Module):
    r''' Initial Reconstruction for Simulated'''
    def __init__(self):
        super(FlatCamSimInitFix, self).__init__()           
...
class FlatCamSimInitConv1(nn.Module):
    r''' Initial Reconstruction of FlatCam by Kronecker Conv'''
    def __init__(self, out_size = 256, height=500, width = 620, in_channels = 4):
        super(FlatCamSimInitConv1, self).__init__()
        self.conv_h1 = nn.Conv2d(1, out_size , kernel_size=[1, width], padding = 0, stride = 1, bias=False)
        self.conv_v1 = nn.Conv2d(1, out_size, kernel_size=[height, 1], padding = 0, stride = 1, bias=False)
        self.conv_h2 = nn.Conv2d(1, out_size , kernel_size=[1, width], padding = 0, stride = 1, bias=False)
        self.conv_v2 = nn.Conv2d(1, out_size, kernel_size=[height, 1], padding = 0, stride = 1, bias=False)
        self.conv_h3 = nn.Conv2d(1, out_size , kernel_size=[1, width], padding = 0, stride = 1, bias=False)
        self.conv_v3 = nn.Conv2d(1, out_size, kernel_size=[height, 1], padding = 0, stride = 1, bias=False)
        self.conv_h4 = nn.Conv2d(1, out_size , kernel_size=[1, width], padding = 0, stride = 1, bias=False)
        self.conv_v4 = nn.Conv2d(1, out_size, kernel_size=[height, 1], padding = 0, stride = 1, bias=False)
        self.conv = nn.Conv2d(in_channels, 3, kernel_size=[3, 3], padding = 1, stride = 1, bias=False)

    def forward(self, x):
        x = Raw2Bayer(x)        
        
        y = F.relu(self.conv_h1(torch.unsqueeze(x[:, 0, :, :], 1)))
        y = F.relu(self.conv_v1(y.permute(0, 3, 2, 1)))        
        out = y.permute(0, 2, 1, 3)

        y = F.relu(self.conv_h2(torch.unsqueeze(x[:, 1, :, :], 1)))
        y = F.relu(self.conv_v2(y.permute(0, 3, 2, 1)))
        out = torch.cat((out, y.permute(0, 2, 1, 3)), 1)

        y = F.relu(self.conv_h3(torch.unsqueeze(x[:, 2, :, :], 1)))
        y = F.relu(self.conv_v3(y.permute(0, 3, 2, 1)))
        out = torch.cat((out, y.permute(0, 2, 1, 3)), 1)

        y = F.relu(self.conv_h4(torch.unsqueeze(x[:, 3, :, :], 1)))
        y = F.relu(self.conv_v4(y.permute(0, 3, 2, 1)))
        out = torch.cat((out, y.permute(0, 2, 1, 3)), 1)
        
        return self.conv(out)

class FlatCamSimInitConv2(nn.Module):
    r''' Initial Reconstruction of FlatCam by Kronecker Conv'''
    def __init__(self, out_size = 256, height=500, width = 620, in_channels = 4):
        super(FlatCamSimInitConv2, self).__init__()
        self.conv_h1 = nn.Conv2d(in_channels, out_size , kernel_size=[1, width], padding = 0, stride = 1, bias=False)
        self.conv_v1 = nn.Conv2d(1, out_size, kernel_size=[height, 1], padding = 0, stride = 1, bias=False)
        self.conv_h2 = nn.Conv2d(in_channels, out_size , kernel_size=[1, width], padding = 0, stride = 1, bias=False)
        self.conv_v2 = nn.Conv2d(1, out_size, kernel_size=[height, 1], padding = 0, stride = 1, bias=False)
        self.conv_h3 = nn.Conv2d(in_channels, out_size , kernel_size=[1, width], padding = 0, stride = 1, bias=False)
        self.conv_v3 = nn.Conv2d(1, out_size, kernel_size=[height, 1], padding = 0, stride = 1, bias=False)
        self.conv_h4 = nn.Conv2d(in_channels, out_size , kernel_size=[1, width], padding = 0, stride = 1, bias=False)
        self.conv_v4 = nn.Conv2d(1, out_size, kernel_size=[height, 1], padding = 0, stride = 1, bias=False)
        self.conv = nn.Conv2d(in_channels, 3, kernel_size=[3, 3], padding = 1, stride = 1, bias=False)
    ...
    def forward(self, x):
        ...
        y = F.relu(self.conv_h1(x))
        y = F.relu(self.conv_v1(y.permute(0, 3, 2, 1)))        
        out = y.permute(0, 2, 1, 3)

        y = F.relu(self.conv_h2(x))
        y = F.relu(self.conv_v2(y.permute(0, 3, 2, 1)))
        out = torch.cat((out, y.permute(0, 2, 1, 3)), 1)

        y = F.relu(self.conv_h3(x))
        y = F.relu(self.conv_v3(y.permute(0, 3, 2, 1)))
        out = torch.cat((out, y.permute(0, 2, 1, 3)), 1)

        y = F.relu(self.conv_h4(x))
        y = F.relu(self.conv_v4(y.permute(0, 3, 2, 1)))
        out = torch.cat((out, y.permute(0, 2, 1, 3)), 1)
        
        return self.conv(out)

class FlatCamSimInitConv3(nn.Module):
    r''' Initial Reconstruction of FlatCam by Kronecker Conv'''
    def __init__(self, out_size = 256, height=500, width = 620, in_channels = 4):
        super(FlatCamSimInitConv3, self).__init__()
        self.conv_h1 = nn.Conv2d(in_channels, out_size , kernel_size=[1, width], padding = 0, stride = 1, bias=False)
        self.conv_v1 = nn.Conv2d(1, out_size, kernel_size=[height, 1], padding = 0, stride = 1, bias=False)
        self.conv_h2 = nn.Conv2d(in_channels, out_size , kernel_size=[1, width], padding = 0, stride = 1, bias=False)
        self.conv_v2 = nn.Conv2d(1, out_size, kernel_size=[height, 1], padding = 0, stride = 1, bias=False)
...
class FlatCamSimInitConv4(nn.Module):
    r''' Initial Reconstruction of FlatCam by Kronecker Conv'''
    def __init__(self, out_size = 64, in_channels = 4):
        super(FlatCamSimInitConv4, self).__init__()
        
        self.conv1 = nn.Conv2d(in_channels, out_size, kernel_size=[3, 3], padding = 1, stride = 1, bias=False)
        self.conv2 = nn.Conv2d(out_size, 3, kernel_size=[3, 3], padding = 1, stride = 2, bias=False)

    def forward(self, x):
        x = Raw2Bayer(x)        
        x = F.relu(self.conv1(x))        
        x = F.relu(self.conv2(F.interpolate(x, size = [512, 512]) ))
