...
class projection_conv(nn.Module):
    def __init__(self, in_channels, out_channels, scale , up = True):
        ...
        if up : 
            self.conv_2 = nn.Conv2d(in_channels, out_channels * ratio, 3,
                                    stride=1, padding=1 )
            self.conv_1 = nn.Conv2d(out_channels, out_channels, 3,
                                    stride=1, padding=1 )
        else: 
            self.conv_1 = nn.Conv2d(in_channels * ratio, out_channels, 3,
                                    stride=1, padding=1 )

    def forward(self, x):       
        if self.up:
            x = self.conv_2(x)   
            for _ in range(0, self.scale):
                x = self.IWT(x)              
