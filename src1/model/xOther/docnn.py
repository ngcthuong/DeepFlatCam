...
class BSR(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = self.d_l1(self.head(self.FDoC1(x)))
        x2 = self.d_l2(self.FDoC2(x1))
        # x3 = self.d_l2(self.DWT(x2))
        x_ = self.IDoC1(self.pro_l3(self.FDoC3(x2))) + x2
        x_ = self.IDoC2(self.i_l2(x_)) + x1

        # x = self.i_l0(x) + x0
        x = self.IDoC3(self.tail(self.i_l1(x_))) + x
        # x = self.add_mean(x)
